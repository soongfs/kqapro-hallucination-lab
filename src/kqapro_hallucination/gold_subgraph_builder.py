...
def build_gold_subgraphs_df(
    ...
    if checkpoint:
        ...
        start = 0
        subgraphs, heads_list, tails_list, entities_list = [], [], [], []
    ...
    for i in tqdm(range(start, total), desc="Extracting subgraphs", initial=start, total=total):
        ...
        if (i + 1) % save_every == 0:
            _save_checkpoint(
                checkpoint_path, i + 1, subgraphs, heads_list, tails_list, entities_list
            )

    _save_checkpoint(
        checkpoint_path, total, subgraphs, heads_list, tails_list, entities_list
    )
